...
export default function HomePage() {
  ...
  const tenantQuery = `?workspace=${encodeURIComponent(publicWorkspaceSlug())}`;

  const { data } = useQuery({
    queryKey: ["homePageData", publicWorkspaceSlug()],
    queryFn: async () => {
      const [cmsPages, workspace] = await Promise.all([store.listCmsPages(), store.getWorkspace()]);
      return { cmsPages, workspace };
    },
  });

  const home = data?.cmsPages.find((p) => p.id === "home");
  const on = (id: string) => home?.sections.find((s) => s.id === id)?.enabled ?? true;
  const guide = data?.workspace.guide;

  if (!data || !guide) return null;

  return (
    <div style={{ ["--orange" as string]: data.workspace.brand.primaryColor, ["--ink" as string]: data.workspace.brand.inkColor, ["--paper" as string]: data.workspace.brand.paperColor }}>
      <SiteNav workspace={data.workspace} />
        ...
        <section className="hero">
          ...
          <div className="hero-inner">
            ...
            <div className="frustration-grid">
              {frustrations.map((f) => (
                <Link key={f.name} href={`${f.href}${tenantQuery}`} className="frustration-item">
            ...
            <div className="hero-cta-block">
              ...
              <Link href={`/f/lack-of-control${tenantQuery}`} className="btn-primary">
          ...
          <div className="howwho-inner">
            ...
            <div id="who" className="who">
              ...
              <figure className="inline-quote">
                ...
                <div>
                  <blockquote>
                    {data.workspace.name} has a unique ability to simplify complex concepts and keep us
        ...
        <section className="guide-band">
          <div className="guide-band-inner">
            <div className="ring-photo">
              ...
              <div className="arc" />
              <div className="photo">
                <img src={guide.photoUrl} alt="Your guide" width={170} height={170} />
              </div>
            </div>
            <div className="guide-band-copy">
              <p className="kicker">Behind {data.workspace.name}</p>
            ...
            <Link href={`/guide${tenantQuery}`} className="btn-ink-outline">
              Meet Your Guide
            </Link>
          </div>
        </section>
      ...
      {on("results") && (
        <section id="results" className="perspectives">
          <div className="perspectives-inner">
            ...
            <p className="perspectives-lede">What clients say about working with {data.workspace.name}.</p>
      ...
      {on("footer") && <SiteFooter variant="home" workspace={data.workspace} />}
    </div>
  );
}
