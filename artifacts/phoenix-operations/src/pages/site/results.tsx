...
export default function ResultsPage() {
  const { data: workspace } = useQuery({
    queryKey: ["results-workspace", publicWorkspaceSlug()],
    queryFn: () => getPublicStore().getWorkspace(),
  });
    ...
    <div style={{ ["--orange" as string]: workspace.brand.primaryColor, ["--ink" as string]: workspace.brand.inkColor, ["--paper" as string]: workspace.brand.paperColor }}>
      <header className="results-header">
        <div className="results-header-inner">
          <Link href={`/${tenant}`} style={{ display: "flex" }}>
            <img
          ...
          <span className="private-badge">Private link · Shared by {workspace.name}</span>
        </div>
      </header>
      ...
      <section className="results-hero">
        ...
        <p>
          {workspace.name} coaches companies on EOS® — the Entrepreneurial Operating System. It
      ...
      <section className="phases">
        <div className="phases-inner">
          ...
          <p className="honest-caveat">
            ...
            accountable. That discipline is exactly what {workspace.name} coaches.
          </p>
        </div>
      </section>
      ...
      <SiteFooter variant="results" workspace={workspace} />
    </div>
  );
}
