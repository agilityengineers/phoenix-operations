...
export default function IntakeExperience({ funnel, variant, guide, workspace }: Props) {
  ...
  const bookingTokenRef = useRef<string | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    if (saved?.resumeToken) {
      ...
      bookingTokenRef.current = saved.bookingToken;
  ...
  const persist = useCallback(
    (next: Partial<Persisted>) => {
      const data: Persisted = {
        ...
        bookingToken: bookingTokenRef.current,
        utm: utmRef.current,
        ...next,
      };
      try {
      ...
      syncTimer.current = setTimeout(() => {
        void apiRequest("/intake/session", {
          method: "POST",
          body: JSON.stringify({
            workspace: publicWorkspaceSlug(),
  ...
  const goNext = async () => {
    ...
    // Final submit — server computes the score and creates the lead.
    if (submitting) return;
    setSubmitting(true);
    try {
      const response = await apiRequest<{ bookingToken: string }>("/intake/submit", {
        method: "POST",
        body: JSON.stringify({ workspace: publicWorkspaceSlug(), funnelSlug: funnel.slug, variant: variant.id, resumeToken: tokenRef.current, answers: a, utm: utmRef.current, website: honeypot }),
      });
      bookingTokenRef.current = response.bookingToken;
    } catch {
      setValidationMsg("We couldn't submit your details. Please try again.");
      setSubmitting(false);
      return;
    }
    setSubmitted(true);
    setSubmitting(false);
    persist({ submitted: true, bookingToken: bookingTokenRef.current });
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", ["--orange" as string]: workspace.brand.primaryColor, ["--ink" as string]: workspace.brand.inkColor, ["--paper" as string]: workspace.brand.paperColor }}>
      <header className="fnl-header">
        <div className="fnl-header-inner">
          <Link href={`/?workspace=${encodeURIComponent(publicWorkspaceSlug())}`} style={{ display: "flex" }}>
            <img
            ...
            <section className="fnl-hero">
              ...
              <div className="stakes-card">
                ...
                <div className="guide-note">
                  <img src={guide.photoUrl} alt={guide.name} width={56} height={56} style={{ borderRadius: '50%' }} />
                  <p>
      ...
      <footer className="fnl-footer">
        <div className="fnl-footer-inner">
          <span>© 2026 {workspace.name}. All rights reserved.</span>
